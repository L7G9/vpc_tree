...
class ASGTree():
    """Get details of AWS Auto Scaling Groups and represents them in a tree structure.
    ...
    def _filter_by_subnets(self, asgs, vpc_subnet_ids):
        """Filter all Auto Scaling Groups by Subnets in the Virtual Private Cloud."""
        filtered_asgs = []
        for asg in asgs:
    ...
    def _subnet_text(self, prefix, subnet_id):
        """Describe a Subnet linked to an Auto Scaling Group as a list of strings."""
        return [f"{get_prefix(prefix)} {subnet_id}"]

    def _instance_text(self, prefix, instance):
        """Describe an Instance linked to an Auto Scaling Group as a list of strings."""
